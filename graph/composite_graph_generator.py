...
class CompositeGraphGenerator(GraphGenerator, NodeLabelAssigner, NodeRemover):
    ...
    def __init__(self, motifs: List[List] = None, seed: int = None, rng: Optional[random.Random] = None):
        # seed and rng parameters are accepted for API compatibility but ignored.
        # Use a single global seed defined by the user in `test_graph.py` (call `random.seed(...)`).
        ...
        # legacy seed/rng fields removed — use global random seed
    ...
    def generate_graph(self, num_extra_vertices: int = 0, num_extra_edges: int = 0, **kwargs) -> nx.Graph:
        ...
        # Composition uses global randomness (seed should be set in `test_graph.py`).
    ...
    def remove_important_nodes(self, graph: nx.Graph, n: int) -> Optional[Tuple[nx.Graph, List[int]]]:
        ...
        # Use global randomness for removal (seed should be set in `test_graph.py`).
